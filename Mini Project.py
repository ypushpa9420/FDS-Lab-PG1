# -*- coding: utf-8 -*-
"""
Created on Sat Jun 19 22:25:39 2021
...
# # handle outlier
# outlierList=["campaign","pdays","previous"]
# for i in outlierList:
#     lol, uol = utils.OutlierLimits(df[i])
#     print("Lower Outlier Limit for :" + str(i) +" is " + str(lol))
#     print("Upper Outlier Limit:" + str(i) + " is " + str(uol))
#     df[i] = np.where(df[i] < lol, lol, df[i])
#     df[i] = np.where(df[i] > uol, uol, df[i])
...
dfp = pd.read_csv('./bank-additional.csv', delimiter=';')
print(dfp.info())
